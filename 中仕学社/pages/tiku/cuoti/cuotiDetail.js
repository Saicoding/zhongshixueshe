...
Page({
    ...
    if (direction == "左滑") {
      ...
      if (px % 10 >= 6) { //滑动到号大于7，这时判断有没有下一个page
        ...
        if (pageArray.indexOf(nextPage) == -1 && nextPage <= pageall) { //已渲染数组不包含下一页面
          ...
          app.post(API_URL, "action=getErrList&zcode=" + zcode + "&typesid=" + options.types + "&token=" + token + "&title=" + title + "&page=" + nextPage, false, false, "", true, self).then((res) => {
      ...
      if (px % 10 <= 4) { //滑动到小于等于3时，这时判断有没有上一个page
        ...
        if (pageArray.indexOf(prePage) == -1 && prePage >= 1) { //已渲染数组不包含下一页面
          ...
          app.post(API_URL, "action=getErrList&zcode=" + zcode + "&typesid=" + options.types + "&token=" + token + "&title=" + title + "&page=" +  prePage, false, false, "", true, self).then((res) => {
  ...
  onUnload:function(){
    ...
    tis[index].nums = nums - rightNum;
    prepage.setData({
      tis:tis,
      all_nums: prepage.data.all_nums - rightNum
    })
  },
  ...
  onShow: function (e) {
    ...
    app.post(API_URL, "action=getErrList&zcode=" + zcode + "&typesid=" + options.types + "&token=" + token +"&title="+title+ "&page=" + page, false, false, "", "", false, self).then((res) => {
      ...
      if (px % 10 >= 1 && px % 10 <= 4 && prepage >= 1) { //px为前半部分并且有上一页时，请求上一页
        app.post(API_URL, "action=getFavoriteShiti&zcode=" + zcode + "&typesid=" + options.types + "&token=" + token + "&page=" + prepage, false, false, "", "", false, self).then((res) => {
          ...
          post.wrongOnload(options, px, circular, myFavorite, shitiArray, user, page, colors, category, all_nums, pageall, self) //对数据进行处理和初始化
        })
        ...
        app.post(API_URL, "action=getFavoriteShiti&zcode=" + zcode + "&typesid=" + options.types + "&token=" + token + "&page=" + nextPage, false, false, "", "", false, self).then((res) => {
          ...
          post.wrongOnload(options, px, circular, myFavorite, shitiArray, user, page, colors, category, all_nums, pageall, self) //对数据进
        })
      } else {
        self.setData({
          pageArray: pageArray
   ...
   * 得到新一组试题
   */
  getNewShiti: function (zcode, options,title, token, page, midShiti, preShiti, nextShiti, px, current, circular) {
    let self = this;
    let shitiArray = self.data.shitiArray;

    app.post(API_URL, "action=getErrList&zcode=" + zcode + "&typesid=" + options.types + "&token=" + token + "&title=" + title + "&page=" +  page, false, false, "", true, self).then((res) => {
      let newWrongShitiArray = res.data.data[0].list;
  ...
  _tapEvent: function (e) {
    ...
    if (pageArray.indexOf(page) == -1) {
      ...
      if (px % 10 >= 1 && px % 10 <= 4 && prepage >= 1 && pageArray.indexOf(prepage) == -1) { //如果是页码的第一题,并且有上一页,并且不在已渲染数组中
        ...
        self.getNewShiti(zcode, options, title, token, page, midShiti, preShiti, nextShiti, px, current, circular);
        self.getNewShiti(zcode, options, title, token, prepage, midShiti, preShiti, nextShiti, px, current, circular);
        ...
        self.getNewShiti(zcode, options, title, token, page, midShiti, preShiti, nextShiti, px, current, circular);
        self.getNewShiti(zcode, options, title, token, nextPage, midShiti, preShiti, nextShiti, px, current, circular);
